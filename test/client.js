...
var ExpressApp = BCHWS.ExpressApp;
var Storage = BCHWS.Storage;
...
helpers.toSatoshi = function(bch) {
  if (_.isArray(bch)) {
    ...
    return parseFloat((bch * 1e8).toPrecision(12));
  }
};
...
helpers.generateUtxos = function(scriptType, publicKeyRing, path, requiredSignatures, amounts) {
  ...
  var utxos = _.map(amounts, function(amount, i) {
    ...
    var address = Utils.deriveAddress(scriptType, publicKeyRing, path, requiredSignatures, Constants.LIVENET);

    var scriptPubKey;
    switch (scriptType) {
      case Constants.SCRIPT_TYPES.P2SH:
        scriptPubKey = bchLib.Script.buildMultisigOut(address.publicKeys, requiredSignatures).toScriptHashOut();
        break;
    ...
    var obj = {
      txid: bchLib.crypto.Hash.sha256(new Buffer(i)).toString('hex'),
      vout: 100,
...
blockchainExplorerMock.setUtxo = function(address, amount, m, confirmations) {
  ...
  switch (address.type) {
    case Constants.SCRIPT_TYPES.P2SH:
      scriptPubKey = address.publicKeys ? bchLib.Script.buildMultisigOut(address.publicKeys, m).toScriptHashOut() : '';
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      scriptPubKey = bchLib.Script.buildPublicKeyHashOut(address.address);
      break;
  ...
  blockchainExplorerMock.utxos.push({
    txid: bchLib.crypto.Hash.sha256(new Buffer(Math.random() * 100000)).toString('hex'),
...
blockchainExplorerMock.broadcast = function(raw, cb) {
  ...
  return cb(null, (new bchLib.Transaction(raw)).id);
};
...
describe('client API', function() {
  ...
  beforeEach(function(done) {
    ...
    expressApp.start({
      ...
      function() {
        ...
        if (!process.env.BCHWC_SHOW_LOGS) {
  ...
  describe('Client Internals', function() {
    it('should expose bch', function() {
      should.exist(Client.bchLib);
      should.exist(Client.bchLib.HDPublicKey);
    });
  });
  ...
  describe('Build & sign txs', function() {
    ...
    var derivedPrivateKey = {
      'BIP44': new bchLib.HDPrivateKey(masterPrivateKey).deriveChild("m/44'/1'/0'").toString(),
      'BIP45': new bchLib.HDPrivateKey(masterPrivateKey).deriveChild("m/45'").toString(),
      'BIP48': new bchLib.HDPrivateKey(masterPrivateKey).deriveChild("m/48'/1'/0'").toString(),
    };

    describe('#buildTx', function() {
      it('Raw tx roundtrip', function() {
        var toAddress = 'CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL';
        var changeAddress = 'CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL';

        var publicKeyRing = [{
          xPubKey: new bchLib.HDPublicKey(derivedPrivateKey['BIP44']),
        }];
        ...
        var t2 = new bchLib.Transaction(t);
      ...
      it('should build a tx correctly (BIP44)', function() {
        ...
        var bchError = t.getSerializationError({
          disableIsFullySigned: true,
          disableSmallFees: true,
          disableLargeFees: true,
        });

        should.not.exist(bchError);
      ...
      it('should build a tx correctly (BIP48)', function() {
        ...
        var publicKeyRing = [{
          xPubKey: new bchLib.HDPublicKey(derivedPrivateKey['BIP48']),
        }];
      ...
      it('should protect from creating excessive fee', function() {
        ...
        var x = Utils.newBchTransaction;

        Utils.newBchTransaction = function() {
          return {
            from: sinon.stub(),
            to: sinon.stub(),
            change: sinon.stub(),
      ...
      it('should build a tx with provided output scripts', function() {
        ...
        var txp = {
          ...
          type: 'external',
          outputs: [{
            "toAddress": "CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL",
            "amount": 700,
        ...
        var changeScript = bchLib.Script.fromAddress(txp.changeAddress.address).toHex();
      ...
      it('should fail if provided output has no either toAddress or script', function() {
        ...
        txp.outputs[0].toAddress = "CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL";
        var t = Utils.buildTx(txp);
        var bchError = t.getSerializationError({
          disableIsFullySigned: true,
        });
    ...
    describe('#signTxp', function() {
      it('should sign BIP45 P2SH correctly', function() {
        ...
        var publicKeyRing = [{
          xPubKey: new bchLib.HDPublicKey(derivedPrivateKey['BIP45']),
        }];
      ...
      it('should sign BCH proposal correctly', function() {
        ...
        signatures[0].should.equal('304402204ad7518c1d16b86655bc7f3f45084aedbba6e61c9a4ad3c2e8e45b7e0fc0d1c102202be01db5d608cf5f69c78482ecae11b6a8406721f3cea1b2c5d0c4fa5e9882df');
        signatures[1].should.equal('304402200b23c27cfefa201c720b8997674abaad4ffa58401503abcba9b07935f8c34964022020935f361a9eebaca6123301ed4dedef04ca60ce1794ea5078ef8a04d42b1b3a');
      });
    });
  });
  ...
  describe('Wallet secret round trip', function() {
    it('should create secret and parse secret', function() {
      ...
      while (i++ < 100) {
        ...
        var walletPrivKey = new bchLib.PrivateKey();
  ...
  describe('Wallet Creation', function() {
    ...
    it('should detect wallets missing callers pubkey', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        helpers.tamperResponse([clients[0], clients[1]], 'get', '/v1/wallets/', {}, function(status) {
          ...
          status.wallet.copayers[1].xPubKey = (new bchLib.HDPrivateKey()).publicKey;
    ...
    it('should set walletPrivKey from BCHWS', function(done) {
      clients[0].createWallet('mywallet', 'creator', 1, 1, {
        network: Constants.LIVENET
      }, function(err) {
  ...
  describe('#getMainAddresses', function() {
    ...
    it('Should return only main addresses when change addresses exist', function(done) {
      var opts = {
        amount: 0.1e8,
        toAddress: 'CfNCvxmKYzZsS78pDKKfrDd2doZt3w4jUs',
        message: 'hello 1-1',
      };
  ...
  describe('Version', function() {
    it('should get version of bchws', function(done) {
      clients[0].credentials = {};
      clients[0].getVersion(function(err, version) {
        if (err) {
          // if bchws is older version without getVersion support
          err.should.be.an.instanceOf(Errors.NOT_FOUND);
        } else {
          // if bchws is up-to-date
          should.exist(version);
          should.exist(version.serviceVersion);
          version.serviceVersion.should.contain('bchws-');
        }
        done();
      });
    });
    ...
    it('should be able to create address in all copayers in a 2-3 wallet', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 2, 3, function() {
        clients[0].createAddress(function(err, x) {
          should.not.exist(err);
          should.exist(x.address);
          x.address.charAt(0).should.equal('H');
          clients[1].createAddress(function(err, x) {
            should.not.exist(err);
            should.exist(x.address);
  ...
  describe('Transaction Proposals Creation and Locked funds', function() {
    ...
    it('Should protect against tampering at proposal creation', function(done) {
      ...
      var tamperings = [
        ...
        function(txp) {
          txp.changeAddress.address = 'CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL';
        },
      ];
    ...
    describe('Detecting tampered tx proposals', function() {
      ...
      it('should detect change address not it wallet', function(done) {
        ...
        helpers.createAndPublishTxProposal(clients[0], opts, function(err, x) {
          ...
          helpers.tamperResponse(clients[0], 'get', '/v1/txproposals/', {}, function(txps) {
            txps[0].changeAddress.address = 'CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL';
          }, function() {
            clients[0].getTxProposals({}, function(err, txps) {
              should.exist(err);
              err.should.be.an.instanceOf(Errors.SERVER_COMPROMISED);
    ...
    describe('Shared wallet', function() {
      ...
      it('Should Create and Verify a Tx from PayPro', function(done) {
        clients[1].getTxProposals({}, function(err, txps) {
          ...
          tx.outputs[0].amount.should.equal(1020000);
          tx.outputs[0].toAddress.should.equal('CepUQ7u4XmMpPY9rgBCrgr6eofXqcxUiGv');
          tx.message.should.equal('Payment request for BitPay invoice SsiUBTwf4YxgJqJjE8YbBj for merchant BitGive');
          ...
          clients[0].signTxProposal(txps[0], function(err, xx, paypro) {
            ...
            clients[1].signTxProposal(xx, function(err, yy, paypro) {
              ...
              clients[1].broadcastTxProposal(yy, function(err, zz, memo) {
                ...
                zz.message.should.equal('Payment request for BitPay invoice SsiUBTwf4YxgJqJjE8YbBj for merchant BitGive');
                done();
              });
            });
          });
          ...
          clients[0].signTxProposal(txps[0], function(err, xx, paypro) {
            ...
            clients[1].signTxProposal(xx, function(err, yy, paypro) {
              ...
              clients[1].broadcastTxProposal(yy, function(err, zz, memo) {
                should.not.exist(err);
                var args = http.lastCall.args[0];
                var data = bchPayPro.Payment.decode(args.body);
                var pay = new bchPayPro();
                ...
                amount.low.should.equal(1020000);
                amount.high.should.equal(0);
                var s = refund_to.get('script');
                s = new bchLib.Script(s.buffer.slice(s.offset, s.limit));
                var addr = new bchLib.Address.fromScript(s, Constants.LIVENET);
          ...
          clients[0].signTxProposal(txps[0], function(err, xx, paypro) {
            ...
            clients[1].signTxProposal(xx, function(err, yy, paypro) {
              ...
              clients[1].broadcastTxProposal(yy, function(err, zz, memo) {
                ...
                var tx = new bchLib.Transaction(rawTx);
    ...
    describe('New proposal flow', function() {
      ...
      it('Should Create and Verify a Tx from PayPro', function(done) {
        clients[1].getTxProposals({}, function(err, txps) {
          ...
          var tx = txps[0];
          // From the hardcoded paypro request
          tx.amount.should.equal(1020000);
          tx.outputs[0].toAddress.should.equal('CepUQ7u4XmMpPY9rgBCrgr6eofXqcxUiGv');
          tx.message.should.equal('Payment request for BitPay invoice SsiUBTwf4YxgJqJjE8YbBj for merchant BitGive');
          tx.payProUrl.should.equal('dummy');
          done();
          ...
          clients[0].signTxProposal(x2, function(err, txp) {
            ...
            if (doBroadcast) {
              clients[0].broadcastTxProposal(txp, function(err, txp) {
                ...
                txp.txid.should.equal((new bchLib.Transaction(blockchainExplorerMock.lastBroadcasted)).id);
                done();
              });
            } else {
              done();
  ...
  describe('Mobility, backup & restore', function() {
    describe('Export & Import', function() {
      ...
      describe('Compliant derivation', function() {
        ...
        it('should export & import with mnemonics + BCHWS', function(done) {
          var c = clients[0].credentials;
        ...
        it('should export & import with xprivkey + BCHWS', function(done) {
          var c = clients[0].credentials;
          var walletId = c.walletId;
          var walletName = c.walletName;
          var copayerName = c.copayerName;
    ...
    describe('Mnemonic related tests', function() {
      ...
      it('should fail to import from words if not at BWS', function(done) {
        ...
        importedClient.importFromMnemonic(exported, {
          ...
          importedClient.credentials.xPrivKey.should.equal('xprv9s21ZrQH143K2eJjoDC7EGqJznV73gWbKj6YjUeAAabRCx7XkB1a2FwDeY4bCrRvA5K8SiuLa5HSLdr4TEbBDq8Qz8oANxHYV7omHN5xbHo');
          done();
        });
      });
      it('should fail to import from words if not at BWS, with passphrase', function(done) {
        ...
        importedClient.importFromMnemonic(exported, {
          ...
          importedClient.credentials.xPrivKey.should.equal('xprv9s21ZrQH143K2gVCqPPPx2sEPzgBF3oas6CqaqvV1ZT63GWxJYcahz9t83MmSDC4cKpS7ap4NNCAKcfoSs2B1HBiommTy65qwVKCMQTWHQp');
          done();
        });
      });
    });
    ...
    describe('Recovery', function() {
      ...
      it('should be able to recreate 1-of-1 wallet with external key (m/48) account 2', function(done) {
        clients[0].seedFromExtendedPublicKey('xprv9s21ZrQH143K4X2frJxRmGsmef9UfXhmfL4hdTGLm5ruSX46gekuSTspJX63d5nEi9q2wqUgg4KZ4yhSPy13CzVezAH6t6gCox1DN2hXV3L', 'ledger', 'b0937662dddea83b0ce037ff3991dd', {
          account: 2,
          derivationStrategy: 'BIP48',
        });
        clients[0].createWallet('mywallet', 'creator', 1, 1, {
  ...
  describe('Air gapped related flows', function() {
    ...
    describe('Failure and tampering', function() {
      ...
      it('should be able to detect tampered change address when signing on airgapped client', function(done) {
        bundle.txps[0].changeAddress.address = 'CPWvcVNXKjZGnHdwPMn7WZtpzYSa27cZqL';
        (function() {
          airgapped.signTxProposalFromAirGapped(bundle.txps[0], bundle.encryptedPkr, bundle.m, bundle.n);
        }).should.throw('Fake transaction proposal');
        done();
    ...
    describe('1-1 wallets', function() {
      ...
      it('should add access with copayer name', function(done) {
        ...
        clients[0].addAccess({
          ...
          var k = new bchLib.PrivateKey(key);
          var c = clients[0].credentials;
          c.requestPrivKey = k.toString();
          c.requestPubKey = k.toPublicKey().toString();
  ...
  describe('Sweep paper wallet', function() {
    ...
    it('should get balance from single private key', function(done) {
      var address = {
        address: 'CfNCvxmKYzZsS78pDKKfrDd2doZt3w4jUs',
        type: 'P2PKH',
      };
      helpers.createAndJoinWallet(clients, 1, 1, function() {
    ...
    it('should build tx for single private key', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 1, 1, function() {
        blockchainExplorerMock.setUtxo(address, 123, 1);
        clients[0].buildTxFromPrivateKey('5KjBgBiadWGhjWmLN1v4kcEZqWSZFqzgv7cSUuZNJg4tD82c4xp', 'CXivsT4p9F6Us1oQGfo6oJpKiDovJjRVUE', {}, function(err, tx) {
          should.not.exist(err);
          should.exist(tx);
          tx.outputs.length.should.equal(1);
          var output = tx.outputs[0];
          output.satoshis.should.equal(123 * 1e8 - 10000);
          var script = new bchLib.Script.buildPublicKeyHashOut(bchLib.Address.fromString('CXivsT4p9F6Us1oQGfo6oJpKiDovJjRVUE'));
    ...
    it('should handle tx serialization error when building tx', function(done) {
      ...
      var se = sandbox.stub(bchLib.Transaction.prototype, 'serialize', function() {
    ...
    it('should fail to build tx for single private key if insufficient funds', function(done) {
      ...
      helpers.createAndJoinWallet(clients, 1, 1, function() {
        ...
        clients[0].buildTxFromPrivateKey('5KjBgBiadWGhjWmLN1v4kcEZqWSZFqzgv7cSUuZNJg4tD82c4xp', 'CXivsT4p9F6Us1oQGfo6oJpKiDovJjRVUE', {
          fee: 500
        }, function(err, tx) {
          should.exist(err);
          err.should.be.an.instanceOf(Errors.INSUFFICIENT_FUNDS);
