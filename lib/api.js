...
var BASE_URL = 'http://localhost:3232/bccws/api';
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = Bcccore.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new Bcccore.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new Bcccore.HDPublicKey(c.xPubKey);
...
API.prototype._import = function(cb) {
  ...
  // First option, grab wallet info from BCCWS.
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new Bcccore.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = Bcccore.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = Bcccore.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  var privateKey = new Bcccore.PrivateKey(privateKey);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = Bcccore.Address.fromString(destinationAddress);

        tx = new Bcccore.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
API.prototype._getHeaders = function(method, url, args) {
  var headers = {
    'x-client-version': 'bccwc-' + Package.version,
  };
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = Bcccore.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new Bcccore.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = Bcccore.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = Bcccore.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new Bcccore.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToBcccoreTx = function(txp, t, signatures, xpub) {
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBcccoreTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new Bcccore.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new Bcccore.PrivateKey(walletPrivKey)).toPublicKey().toString(),
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = Bcccore.PrivateKey.fromString(c.walletPrivKey);
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var newClient = new API({
    baseUrl: 'https://bccws.example.com/bccws/api'
  });
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new Bcccore.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new Bcccore.HDPrivateKey(this.credentials.xPrivKey)
