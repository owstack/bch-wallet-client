...
var BASE_URL = 'http://localhost:3232/bchws/api';
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = bchLib.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new bchLib.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new bchLib.HDPublicKey(c.xPubKey);
...
API.prototype._import = function(cb) {
  ...
  // First option, grab wallet info from BCHWS.
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new bchLib.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = bchLib.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = bchLib.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  var privateKey = new bchLib.PrivateKey(privateKey);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = bchLib.Address.fromString(destinationAddress);

        tx = new bchLib.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
API.prototype._getHeaders = function(method, url, args) {
  var headers = {
    'x-client-version': 'bchwc-' + Package.version,
  };
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = bchLib.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new bchLib.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = bchLib.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = bchLib.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new bchLib.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToBchTx = function(txp, t, signatures, xpub) {
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBchTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new bchLib.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new bchLib.PrivateKey(walletPrivKey)).toPublicKey().toString(),
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = bchLib.PrivateKey.fromString(c.walletPrivKey);
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var newClient = new API({
    baseUrl: 'https://bchws.example.com/bchws/api'
  });
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new bchLib.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new bchLib.HDPrivateKey(this.credentials.xPrivKey)
