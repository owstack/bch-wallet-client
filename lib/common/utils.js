...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Bcccore.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Bcccore.PrivateKey.fromString(privKey);
  return Bcccore.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bcccoreAddress = Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bcccoreAddress = Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
Utils.buildTx = function(txp) {
  ...
  // Validate inputs vs outputs independently of Bcccore
