...
var crypto = bchLib.crypto;
var encoding = bchLib.encoding;
...
Utils.hashMessage = function(text) {
  ...
  ret = new bchLib.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(bchLib.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = bchLib.PrivateKey.fromString(privKey);
  return bchLib.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      bchAddress = Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      bchAddress = Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
Utils.buildTx = function(txp) {
  ...
  // Validate inputs vs outputs independently of bchLib
