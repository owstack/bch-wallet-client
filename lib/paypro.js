...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': bchPayPro.PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = bchPayPro.PaymentRequest.decode(dataBuffer);
      request = (new bchPayPro()).makePaymentRequest(body);
    ...
    var decodedDetails = bchPayPro.PaymentDetails.decode(serializedDetails);
    var pd = new bchPayPro();
    ...
    var addr = new bchLib.Address.fromScript(new bchLib.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var output = new bchPayPro.Output();
  var addr = new bchLib.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = bchLib.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = bchLib.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': bchPayPro.PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': bchPayPro.PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = bchPayPro.PaymentACK.decode(rawData);
        var pp = new bchPayPro();
