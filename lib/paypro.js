...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': BcccorePayPro.PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = BcccorePayPro.PaymentRequest.decode(dataBuffer);
      request = (new BcccorePayPro()).makePaymentRequest(body);
    ...
    var decodedDetails = BcccorePayPro.PaymentDetails.decode(serializedDetails);
    var pd = new BcccorePayPro();
    ...
    var addr = new Bcccore.Address.fromScript(new Bcccore.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var output = new BcccorePayPro.Output();
  var addr = new Bcccore.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = Bcccore.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = Bcccore.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': BcccorePayPro.PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': BcccorePayPro.PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = BcccorePayPro.PaymentACK.decode(rawData);
        var pp = new BcccorePayPro();
